...
## 3.5. World Models and Digital Twins for Manipulation

**Learned world models.** An emerging line of work learns pixel-space world models for manipulation planning, predicting future visual observations conditioned on actions. UniSim \cite{yang2024unisim} trains a video generation model as a universal simulator from internet and robot data. IRASim \cite{zhu2024irasim} generates robot manipulation videos conditioned on actions via a diffusion transformer. Genie 2 \cite{deepmind2024genie2} and GameGen \cite{che2024gamegen} learn interactive world models that generate controllable environments. RoboDreamer \cite{zhou2024robodreamer} uses video prediction for model-based planning. GWM \cite{lu2025gwm} learns Gaussian world models for action-conditioned prediction. While these approaches avoid explicit 3D reconstruction, they require large-scale real robot interaction data for training and do not address continual adaptation to evolving environments. **Our approach is complementary: we provide a data generation pipeline that could serve as training data for such world models, while our continual learning framework addresses adaptation as the environment changes — a problem that world models do not solve.**

**Digital twin approaches.** Li et al. \cite{li2026worldmodel} build explicit world models for zero-shot open-world manipulation by reconstructing per-object meshes via Hunyuan3D 2.0 (single-image to textured mesh) \cite{zhao2025hunyuan3d}, placing them in Isaac Sim, and sampling actions via Gaussian Process optimization with GPT-4o as a success judge. Their approach achieves 12.5–62.5% real-world success rates, with mesh alignment failures as the primary bottleneck — a consequence of single-image reconstruction's inherent scale ambiguity, which requires RGB-D + ICP for 7-DOF pose alignment. DexSim2Real2 \cite{jiang2025dexsim2real2} applies similar digital twin reasoning to articulated objects. **Our approach differs fundamentally in purpose: rather than using the digital twin for one-shot action sampling, we use it as a data factory for VLA continual fine-tuning. Our multi-view 3DGS reconstruction provides metric-scale geometry (from COLMAP) without requiring depth sensors, and our learning-based approach accumulates experience across environments rather than planning from scratch each time.**
...
**Synthetic data for robot learning.** Domain randomization \cite{tobin2017domainrandomization} and sim-to-real transfer remain central to robot learning. NVIDIA's GR00T \cite{nvidia2025groot} blueprint generates 780K synthetic trajectories from 11 hours of demonstrations using GR00T-Mimic + Cosmos Transfer. RoboPaint \cite{chen2025robopaint} uses 2D inpainting for visual data augmentation but lacks 3D geometric consistency. **Our approach uses 3DGS as a scene digitizer — reconstructing and segmenting real environments into per-object meshes for import into simulation — combined with sim-based trajectory execution and rendering for physically-grounded data synthesis. This hybrid strategy combines the geometric fidelity of multi-view 3DGS reconstruction with the physical accuracy of simulation.**
...
| Digital twin for manipulation | Li et al. (2026) | + Learning-based (not one-shot planning) + Multi-view 3DGS (metric scale, no RGB-D) + Continual adaptation |
| World models | UniSim, IRASim, GWM | + No large-scale real data needed + Explicit 3D (not pixel-space) + Environment-level CL |
| VLA adaptation | TwinRL-VLA | + No RL reward engineering + 3DGS-based scene digitization |
| Continual manipulation | LIBERO | + Evolving environments (not just new tasks) + EvoHome-Bench |
| CL for VLAs | CRL-VLA, LifeLong-RFT, Simple Recipe | + Environment-level CL (not task-level) + Zero demos + 3DGS → sim pipeline |
