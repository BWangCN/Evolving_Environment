...
**VLA adaptation.** Despite impressive zero-shot generalization, current VLAs struggle with personalized deployment. The BEHAVIOR-1K 2025 Challenge \cite{li2024behavior} showed π0 achieving only ~26% success rate across 50 household tasks. TwinRL-VLA \cite{wang2025twinrl} proposed RL-based adaptation but requires reward engineering. **Our work addresses this gap by proposing zero-interaction adaptation via 3DGS-synthesized data, requiring no additional real-world demonstrations per new environment.**
...
**Generative replay.** Instead of storing raw data, generative replay uses a generative model to synthesize replay samples. This approach has been explored with VAEs \cite{shin2017generativereplay} and GANs for classification CL. **Our 3DGS Environment Bank extends this concept to 3D scenes — previous environments are stored as compact 3DGS representations that can render unlimited novel-view replay samples, avoiding the storage cost of raw images and the distribution collapse of replaying identical training data.**
